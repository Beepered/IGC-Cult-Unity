...
public class EventHandler : MonoBehaviour
{
    ...
    {   
        if (event_turns != null)
        {
            ...
            foreach(string eve in event_turns_copy)
            {
                ...
                if (event_turns[eve] == 0)
                {
                    ...
                    else if (eve == "The Profit’s Paradise")
                    {
                        foreach (Tile t in bp.tiles)
                        {
                            if(t.building != null)
                            {
                                if (t.building.name == "Theater")
                                {
                                    t.building.money -= 5;
                                }
                            }
                    ...
                    else if (eve == "The Profit’s Paradise")
                    {
                        foreach (Tile t in bp.tiles)
                        {
                            if(t.building != null)
                            {
                                if (t.building.name == "Church")
                                {
                                    t.building.insanity -= 5;
                                }
                            }
                        }
                    }
                    else if (eve == "Reviving the Economy")
                    {
                        vc.event_people_mod *= 2;
                        vc.money_mod /= 2;
                    }
        ...
        switch (Random.Range(0, 24))
        {
            ...
            case 12:
                ...
                if (buildings_exist)
                {
                    int random_row = Random.Range(0, 3); //the row could be empty, but I don't feel like checking every row with a building and then random number generating
                    if (random_row == 0)
                    {
            ...
            case 22:
                MoveButton(0);
                event_text.text = "The Profit’s Paradise:\n\n\nThe latest play is a smash hit! Tickets are almost impossible to find. You’ve even gained the attention of some of the most powerful monsters out there: Broadway producers.\n\nTheaters generate +$5 for three turns";
                AddEvent("The Profit’s Paradise", 3);
                break;
            case 23:
                MoveButton(0);
                event_text.text = "No Tent for This Revival:\n\n\nYou’ve been able to recruit a new pastor to the cause, a real fire and brimstone preacher. Just replace fire and brimstone with the Court of Azathoth. Not your cup of tea, but the people seem to love it.\n\nChurches generate extra insanity";
                AddEvent("No Tent for This Revival", 3);
                break;
            case 24:
                MoveButton(0);
                event_text.text = "Reviving the Economy:\n\n\nSome necromancers have decided to set up shop in your town, and they have the unfortunate habit of killing your citizens to use in their rituals. Normally, you would put a stop to this, but their alchemists have driven a hard bargain. What’s a life worth? Enough for a new lair, you hope.\n\nPopulation halved for 3 turns, earn + 50 money for 3 rounds";
                AddEvent("Reviving the Economy", 3);
                break;
        }
    }
    ...
    public void Option1() //button 1
    {
        ...
        vc.can_end_turn = true; //hide button and allow you to press end turn again
    }
    ...
    private void EventEffects()
    {
        foreach (var eve in event_turns)
        {
            ...
            else if (eve.Key == "The Profit’s Paradise")
            {
                foreach (Tile t in bp.tiles)
                {
                    if(t.building != null)
            ...
            else if(eve.Key == "No Tent for This Revival")
            {
                foreach (Tile t in bp.tiles)
                {
                    if(t.building != null)
            ...
            else if (eve.Key == "Reviving the Economy")
            {
                vc.event_people_mod /= 2;
                vc.event_money_mod *= 2;
            }
    ...
    private void EventDestroyBuilding(int number)
    {
        ...
        {
            bp.SellBuilding(bp.tiles[number].building); //give money back
            bp.tiles[number].DestroyBuilding(); //destroy the building on the tile
        }
    }
